...
function get_revision() {
    ...
    here=`pwd`
    if [ "$here" == "$T" ]; then
        rev=`repo info . | grep "Manifest merge branch" | awk {'print $3'} | sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g"`
    else
...
function repoinit() {
    ...
    repo init -u https://github.com/AOKP/platform_manifest.git -b kitkat -g all,-notdefault,${groups}
}
