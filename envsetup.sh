...
function generate_aokp_sdk() {
    ...
    ORIGINAL_SDK=$ANDROID_HOME/platforms/android-19
    TARGET_SDK=$ANDROID_HOME/platforms/android-19.aokp
    ...
    if [ ! -d "$OUT" ] ; then
        echo "Run this script from your build directory after . build/envsetup.sh"
        exit
    fi
    ...
    FRAMEWORKJAR=$JAVA_LIBS/framework_intermediates/classes.jar
    COREJAR=$JAVA_LIBS/core_intermediates/classes.jar
        FRAMEWORKRESJAR=$JAVA_LIBS/framework-base_intermediates/classes.jar
