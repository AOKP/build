...
function generate_aokp_sdk() {
    ...
    if [ ! -d "$TARGET_SDK" ] ; then
        ...
        sed -e 's/ro.build.version.sdk=19/ro.build.version.sdk=119/' $TARGET_SDK/build.prop > /dev/null
        sed -e 's/AndroidVersion.ApiLevel=19/AndroidVersion.ApiLevel=119/' -e 's/Pkg.Desc=/Pkg.Desc=AOKP /' $TARGET_SDK/source.properties > /dev/null
    fi
