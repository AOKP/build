...
function generate_aokp_sdk() {
    ...
    if [ ! -d "$TARGET_SDK" ] ; then
        ...
        sed -i 's/ro.build.version.sdk=19/ro.build.version.sdk=119/' $TARGET_SDK/build.prop
        sed -i 's/AndroidVersion.ApiLevel=19/AndroidVersion.ApiLevel=119/' $TARGET_SDK/source.properties
        sed -i 's/Pkg.Desc=/Pkg.Desc=AOKP /' $TARGET_SDK/source.properties
    fi
